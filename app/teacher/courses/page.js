...
const Page = () => {
  ...
  const myCourses = (courses || []).filter(
    (c) => Array.isArray(c?.teachers) && c.teachers.includes(currentUser?._id)
  );

  // Calculate statistics
  const stats = {
    totalCourses: myCourses.length,
    totalClasses: myCourses.reduce(
      (sum, course) => sum + (course?.classIds?.length || 0),
      0
    ),
    activeCourses: myCourses.filter((course) => course?.classIds?.length > 0)
      .length,
    upcomingCourses: myCourses.filter(
      (course) => !course?.classIds || course.classIds.length === 0
    ).length,
  };
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-2 text-lg">
                  Manage and organize your teaching courses and materials
                </p>
              </div>
            </div>
          ...
          >
            <div className="max-w-md mx-auto">
              ...
              <p className="text-gray-600 text-lg mb-8 max-w-sm mx-auto leading-relaxed">
                ...
                administrator to get started with course assignments.
              </p>

              <div className="flex items-center justify-center gap-4 text-gray-500">
                <div className="flex items-center gap-2">
                  <FiFolder className="w-5 h-5" />
                  <span>Waiting for course assignments</span>
                </div>
              </div>
            </div>
          </motion.div>
                    ...
                    <div className="flex items-start gap-3 mb-4 min-h-[80px]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-600 text-sm line-clamp-2 leading-relaxed">
                          {course?.description || "Course managed by you"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 mb-4 flex-1">
                      <div className="flex items-center justify-between text-sm text-gray-600">
                        <span className="flex items-center gap-2 whitespace-nowrap">
                          ...
                          {course?.classIds?.length || 0} Classes
                        </span>
                        <span className="flex items-center gap-2 whitespace-nowrap">
